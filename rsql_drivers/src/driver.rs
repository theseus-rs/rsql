...
impl DriverManager {
    ...
    pub async fn connect(&self, url: &str) -> Result<Box<dyn Connection>> {
        let parsed_url = Url::parse(url).map_err(|error| InvalidUrl(error.into()))?;
...
impl Default for DriverManager {
    fn default() -> Self {
        ...
        #[cfg(feature = "file")]
        drivers.add(Box::new(rsql_driver_file::Driver));
        #[cfg(feature = "http")]
        drivers.add(Box::new(rsql_driver_http::Driver));
        #[cfg(feature = "https")]
        drivers.add(Box::new(rsql_driver_https::Driver));
        #[cfg(feature = "json")]

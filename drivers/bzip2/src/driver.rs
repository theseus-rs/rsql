...
/// Driver for Bzip2 compressed files.  The driver decompresses the original file and then delegates
...
impl rsql_driver::Driver for Driver {
    ...
    fn supports_file_type(&self, file_type: &FileType) -> bool {
        file_type.extensions().contains(&"bz2")
    }
}
...
impl Driver {
    fn decompress_file(file: &PathBuf, temp_dir: &Path) -> Result<PathBuf> {
        ...
        if let Some(extension) = file_name.extension() {
            if extension.to_string_lossy().to_string().to_lowercase() == "bz2" {

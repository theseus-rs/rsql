...
#[cfg(target_os = "linux")]
#[tokio::test]
async fn test_cratedb_driver() -> anyhow::Result<()> {
    let image =
        testcontainers::ContainerRequest::from(testcontainers_modules::cratedb::CrateDB::default());
    let container = image.start().await?;
    let port = container.get_host_port_ipv4(5432).await?;

    let database_url = format!("cratedb://crate@localhost:{port}/postgres");
    let mut connection = rsql_driver_cratedb::Driver
